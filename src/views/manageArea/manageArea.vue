  ...
  <div class="pad_5">
    ...
    <div class="query_fields">
      ...
      >
        ...
        <el-form-item label="业务区域" prop="management_area">
          <el-select v-model="queryForm.management_area" placeholder="请选择业务区域">
            <el-option
              v-for="(item, index) in queryForm.regions"
              :key="index"
              :label=" item.province "
              :value=" item.adcode "
            ></el-option>
    ...
    <div>
      ...
      <el-table :data="tableData" v-loading="tableLoading" border stripe style="width: 100%">
        ...
        <el-table-column prop="management_area" label="业务区域" width></el-table-column>
      ...
      <div class="wid_b75">
        ...
        >
          ...
          <el-form-item label="区域" prop="region" class="modiWidth_select">
            <el-select
              v-model="add_formInline.region"
              multiple
              placeholder="请选择区域"
              @change="add_changeOption($event)"
            >
              <el-option
                v-for="(item, index) in add_formInline.regions"
                :key="index"
                :label=" item.province "
                :value=" item.adcode "
              ></el-option>
...
export default {
  ...
  methods: {
    ...
    getTableDataList(pageNum) {
      ...
      let param = {
        data: {
          ...
          // 区域
          management_area:
            provinces.province_list[this.queryForm.management_area],
          // 姓名
      ...
      this.$http
        .post(`${commonUrl.baseUrl}/manageArea/selectManageArea`, param)
        .then(res => {
    ...
    handle_modi(row) {
      ...
      this.$http
        .post(`${commonUrl.baseUrl}/manageArea/updateManageArea`, param)
        .then(res => {
          if (res.data.code == "0000") {
            console.log(res);
    ...
    save_add() {
      ...
      if (this.valid_form("add_formInline")) {
        ...
        this.add_loading = true;
        this.$http
          .post(`${commonUrl.baseUrl}/manageArea/saveManageArea`, param)
          .then(res => {
            if (res.data.code == "0000") {
              console.log(res);
              // 成功之后清空一下 modi_id
