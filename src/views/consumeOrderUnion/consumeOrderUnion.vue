  ...
  <div class="pad_5">
    ...
    <div class="query_fields pad_b_no handle_timerange">
      ...
      >
        ...
        <el-form-item label="支付方式" prop="pay_type">
          <el-select v-model="queryForm.pay_type" class="wid_140" placeholder="请选择支付方式">
            <el-option
              v-for="(item, index) in queryForm.pay_types"
              :key="index"
              :label=" item.value "
              :value=" item.id "
            ></el-option>
        ...
        <!-- <el-form-item label="支付类别" prop="consume_type">
          ...
          >
            <el-option
              v-for="(item, index) in queryForm.consume_types"
              :key="index"
              :label=" item.value "
              :value=" item.id "
            ></el-option>
        ...
        <!-- <el-form-item label="支付状态" prop="consume_refund_status">
          ...
          >
            <el-option
              v-for="(item, index) in queryForm.consume_refund_statuss"
              :key="index"
              :label=" item.value "
              :value=" item.id "
            ></el-option>
    ...
    <div>
      ...
      <el-table :data="tableData" v-loading="tableLoading" border stripe style="width: 100%">
        ...
        <!-- 订单时间 -->
        <el-table-column prop="createtime" :show-overflow-tooltip="true" label="订单时间" width></el-table-column>
        <!-- 支付方式  1支付宝 2微信 -->
        ...
        <el-table-column prop label="支付状态" width="80px">
          <template slot-scope="scope">
            ...
            <span
              v-else-if="scope.row.consume_type == 1 && scope.row.consume_refund_status == 1"
            >支付成功</span>
            <span
              v-else-if="scope.row.consume_type == 1 && scope.row.consume_refund_status == 2"
            >支付失败</span>
            <span
              v-else-if="scope.row.consume_type == 1 && scope.row.consume_refund_status == 3"
            >支付超时</span>
            <!-- 退款 -->
            <span v-if="scope.row.consume_type == 2 && scope.row.consume_refund_status == 0">待退款</span>
            <span
              v-else-if="scope.row.consume_type == 2 && scope.row.consume_refund_status == 1"
            >退款成功</span>
            <span
              v-else-if="scope.row.consume_type == 2 && scope.row.consume_refund_status == 2"
            >退款失败</span>
            <span
              v-else-if="scope.row.consume_type == 2 && scope.row.consume_refund_status == 3"
            >退款超时</span>
          </template>
        </el-table-column>
        <!-- 消费的订单编号 -->
...
export default {
  ...
  methods: {
    ...
    getTabelDataList(pageNum) {
      ...
      this.$http
        .post(
          `${commonUrl.baseUrl}/consumeOrderUnion/getConsumeOrderUnion`,
          param
        )
        .then(res => {
