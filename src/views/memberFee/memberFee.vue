    ...
    <div class="pad_5">
      ...
      <div>
          ...
          <el-table :data="tableData" v-loading="tableLoading" border stripe style="width: 100%">
            <el-table-column prop="" label="会员内容" width="100">
              <template slot-scope="scope">
                <span >{{scope.row.memberMonth }}个月会员</span>

              </template>
            </el-table-column>
            <el-table-column prop="money" label="金额" width="80px"></el-table-column>
...
export default {
    ...
    methods:{
      ...
      getTabelDataList(pageNum){
        ...
        this.$http.post(`${ commonUrl.baseUrl }/accountProfit/getMemberFee`, param).then(res=>{
            // console.log(res)
            // debugger
            if(res.data.code == '0000'){
      ...
      exportData(){
          ...
          // 其他收益的数据导出
          window.location.href = `${commonUrl.baseUrl}/accountProfit/exportMemberFee?signInUserId=${param.data.signInUserId}&signInRoleId=${param.data.signInRoleId}&customid=${param.data.customid}&account_class=${param.data.account_class}&agentName=${param.data.agentName}&startTime=${param.data.startTime}&endTime=${param.data.endTime}`
